...
/** Where you are standing, for a layer that asks: any painted pixel under
 *  you. A tag by your dot on the map says `label`; tapped, it opens a card
 *  headed "You're in <what>" -- joined with "and" when you are in more than
 *  one -- over a pill per layer and its `body`. */
const here = z.object({
  label: z.string(),
  /** The layer as a phrase, article and all: "a habitat core". */
  what: z.string(),
  body: z.string(),
  /** "Things to look for": short things a person standing there could
   *  actually notice. A bullet each. */
  look: z.array(z.string()).optional(),
});

/** The map (src/pages/map.astro): every layer of the map portal -- they live
 *  outside this file, in src/data/ecoscapes-layers.json (generated by
 *  tools/fetch-felt-layer*), grouped by src/data/ecoscapes-themes.json --
 *  plus where you are on it, the posts, the stories and the Bear. This file
 *  holds the view it opens on, and this site's own additions. */
const ecoscapes = defineCollection({
  ...
  schema: z.object({
    ...
    /** The basemap PMTiles file, under public/ -- the whole EcoScapes study
     *  area, cut by tools/extract-basemap.sh. */
    basemap: z.string(),
    ...
    intro: z.string(),
    /** The Bear's page, opened from its pill. */
    bear: z.object({
      title: z.string(),
      body: z.string(),
      image: z.string().optional(),
    }),
    /** Laid over the portal's layer with the same id; any other id is a layer
     *  of this site's own, stacked over the portal's in this order (the last
     *  on top), and needs `name`, `src`, `theme` and `subtheme`. */
    layers: z
      .array(
        z.object({
          ...
          /** Under public/: a PMTiles file from tools/prepare-layer.sh. */
          ...
          resampling: z.enum(['nearest', 'linear']).optional(),
          /** How the legend reads: a ramp for classes or a surface, lowest
           *  first; a swatch list for categories. */
          kind: z.enum(['classes', 'categories', 'continuous']).optional(),
          theme: z.string().optional(),
          subtheme: z.string().optional(),
          /** One plain line in the legend saying what the layer is. */
          about: z.string().optional(),
          /** Lowest first, as the portal's are. */
          legend: z.array(z.object({ colour: z.string(), label: z.string() })).optional(),
        }),
      )
      .default([]),
    /** Headings the layer list needs that the portal's own does not have. */
    subthemes: z.array(z.object({ theme: z.string(), id: z.string(), name: z.string() })).default([]),
...
export const collections = { locations, ecoscapes };
